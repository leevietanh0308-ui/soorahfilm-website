...
import { evaluateShot, initialShot, scenePresets } from "@/lib/film-lab";
import type { CameraType, FlashMode, RiskBand, SceneId, ShotInput, ShotResult } from "@/lib/film-lab";
...
const flashLabels: Record<ShotResult["flashRisk"], string> = { low: "CÓ THỂ HỮU ÍCH", moderate: "CHƯA CHẮC", high: "QUÁ XA", "not-used": "KHÔNG BẬT" };

function RiskRow({ label, risk, display }: { label: string; risk: RiskBand | "not-used"; display: string }) {
  return <div className="lab-risk-row"><span>{label}</span><strong className={`lab-risk-${risk}`}>{display}<span className="lab-risk-dot" aria-hidden="true" /></strong></div>;
}
...
export default function FilmLab() {
  ...
  return <div className="film-lab-page">
    ...
    <section className="lab-workbench section-pad" id="kiem-tra" aria-labelledby="workbench-title">
      <div className="section-index"><span>01 / THỬ NGHIỆM</span><span>CHỈ CẦN NHỮNG GÌ BẠN BIẾT</span></div>
      <div className="lab-workbench-head"><div><span className="eyebrow">BEGINNER MODE / ULTRAMAX 400</span><h2 id="workbench-title">Bạn định<br /><em>chụp gì?</em></h2></div><p>Mỗi thay đổi cập nhật lời khuyên ngay. Không cần biết khẩu độ, tốc độ hay EV để bắt đầu.</p></div>
      <div className="lab-grid">
        <div className="lab-controls">
          ...
          <button className="lab-reset" type="button" onClick={() => { setShot(initialShot); setShowBefore(false); track("film_lab_change", { field: "reset", value: "initial" }); }}><RotateCcw size={14} aria-hidden="true" /> THỬ LẠI TỪ ĐẦU</button>
        </div>

        <div className="lab-output" aria-label="Kết quả ước tính">
          <div className="lab-preview-header"><span>02 / KHUNG HÌNH MINH HỌA</span><span>LIVE ESTIMATE <span className="lab-live-dot" aria-hidden="true" /></span></div>
          <div className={`lab-preview lab-tone-${showBefore ? "balanced" : result.previewTone}`}><Image src="/images/film/frame-17.webp" alt="Ảnh film mẫu của SOORAH: một nhóm người ngoài trời" fill sizes="(max-width: 850px) 100vw, 48vw" quality={78} /><div className="lab-preview-shade" /><span className="lab-preview-index">SOORAH / 01—36</span><span className="lab-preview-mark">◻</span><span className="lab-preview-caption">{showBefore ? "ẢNH MẪU GỐC" : "MINH HỌA XU HƯỚNG ÁNH SÁNG"}</span></div>
          <div className="lab-preview-foot"><p>Ảnh mẫu chỉ minh họa độ sáng. Đây không phải bối cảnh bạn đã chọn hay ảnh dự đoán từ máy của bạn.</p><button type="button" onClick={() => setShowBefore((value) => !value)} aria-pressed={showBefore}>{showBefore ? "XEM ƯỚC TÍNH" : "SO ẢNH GỐC"} ↗</button></div>

          <div className="lab-verdict" aria-live="polite"><span className="eyebrow">KẾT QUẢ CÓ KHẢ NĂNG / {scene.short}</span><h3>{result.headline}</h3><p>{result.description}</p><div className="lab-risk-list"><RiskRow label="Rủi ro thiếu sáng" risk={result.exposureRisk} display={riskLabels[result.exposureRisk]} /><RiskRow label="Rủi ro nhòe" risk={result.motionRisk} display={riskLabels[result.motionRisk]} /><RiskRow label="Flash" risk={result.flashRisk} display={flashLabels[result.flashRisk]} /></div><p className="lab-certainty">{result.certainty === "limited" ? "Mức chắc chắn: giới hạn — thiếu thông tin về máy hoặc flash AUTO." : "Mức chắc chắn: theo tình huống — thông số máy thực tế vẫn có thể khác."}</p></div>

          <div className="lab-reach"><div><span className="eyebrow">VÙNG FLASH GẦN / MINH HỌA</span><strong>{shot.distance === 20 ? "20 m trở lên" : `${shot.distance} m`}</strong></div><div className="lab-reach-track" aria-hidden="true"><div className="lab-reach-near" /><span className="lab-reach-marker" style={{ left: `${({ 0.5: 7, 1: 13, 2: 25, 3: 36, 5: 53, 10: 75, 20: 94 } as Record<number, number>)[shot.distance]}%` }} /></div><div className="lab-reach-labels"><span>0 — 3 M / GẦN</span><span>XA HƠN</span></div><p>Khoảng 3 m chỉ là mốc tham khảo để học. Hiệu quả flash thật phụ thuộc từng máy, pin và điều kiện chụp.</p></div>
        </div>
      </div>
    </section>
    ...
    <section className="lab-actions section-pad" aria-labelledby="actions-title"><div className="section-index"><span>03 / TRƯỚC KHI BẤM</span><span>THAY MỘT ĐIỀU</span></div><div className="lab-actions-head"><div><span className="eyebrow">HOW TO IMPROVE THIS SHOT?</span><h2 id="actions-title">Bạn có thể<br /><em>đổi điều gì?</em></h2></div><p>Thử thay một biến rồi xem mức rủi ro bên trên thay đổi ra sao.</p></div><div className="lab-suggestions">{result.suggestions.map((suggestion, index) => <article key={suggestion}><span>0{index + 1} / GỢI Ý</span><p>{suggestion}</p><Check size={19} aria-hidden="true" /></article>)}</div><div className="lab-quick-actions"><button type="button" disabled={shot.light >= 5} onClick={() => update("light", Math.min(5, shot.light + 1))}><Lightbulb size={17} aria-hidden="true" /> THÊM ÁNH SÁNG</button><button type="button" disabled={shot.flash === "on"} onClick={() => update("flash", "on")}>✳ BẬT FLASH</button><button type="button" disabled={shot.distance <= 0.5} onClick={() => update("distance", shot.distance > 3 ? 3 : shot.distance > 1 ? 1 : 0.5)}>↘ TIẾN GẦN HƠN</button></div></section>
